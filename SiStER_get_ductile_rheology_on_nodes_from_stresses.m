...
if PARAMS.YNThreshold == 1
	eta_high = permute(repmat([MAT.eta_high]', [1,size(T)]), [2,3,1]);
	eta_low = permute(repmat([MAT.eta_low]', [1,size(T)]), [2,3,1]);
	thresh = permute(repmat([MAT.threshold]', [1,size(T)]), [2,3,1]);
	
    sII = repmat(sII, [1, 1, PARAMS.Nphase]);
    T = repmat(T, [1, 1, PARAMS.Nphase]);

	[eta_thresh] = SiStER_flow_law_function('threshold',pre_diff,Ediff,ndiff,PARAMS.R,T,zeros(size(sII)),sII,PARAMS, eta_high, eta_low, thresh);
	eta = sum(eta_thresh.*phase_node, 3);
% 	eta = eta_thresh;
else
